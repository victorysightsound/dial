...
test("buildDownloadUrl uses release defaults", () => {
  const url = buildDownloadUrl("4.2.3", "dial-x86_64-apple-darwin.tar.gz", {});
  assert.equal(
    url,
    "https://github.com/victorysightsound/dial/releases/download/v4.2.3/dial-x86_64-apple-darwin.tar.gz",
  );
});

test("buildDownloadUrl respects overrides", () => {
  const url = buildDownloadUrl("4.2.3", "dial.zip", {
    DIAL_NPM_BASE_URL: "https://example.com/releases/v4.2.3/",
  });
  assert.equal(url, "https://example.com/releases/v4.2.3/dial.zip");

  const directUrl = buildDownloadUrl("4.2.3", "ignored.zip", {
