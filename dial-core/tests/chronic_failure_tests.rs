...
impl EventHandler for ChronicEventCollector {
    fn handle(&self, event: &Event) {
        if let Event::ChronicFailureDetected { task_id, total_failures, total_attempts } = event {
            self.events.lock().unwrap().push((*task_id, *total_failures, *total_attempts));
        }
    }
}
...
async fn test_create_iteration_increments_total_attempts() {
    ...
    let task_id = engine.task_add("attempt counter task", 5, None).await.unwrap();
...
async fn test_complete_iteration_failed_increments_total_failures() {
    ...
    let task_id = engine.task_add("failure counter task", 5, None).await.unwrap();
    ...
    dial_core::iteration::complete_iteration(&conn, iteration_id, "failed", None, Some("test failure")).unwrap();
    ...
    let last_failure: Option<String> = conn.query_row(
...
async fn test_auto_block_chronic_failure_integration() {
    ...
    let task_id = engine.task_add("chronically failing task", 5, None).await.unwrap();
    ...
    for attempt in 1..=12 {
        ...
        dial_core::iteration::complete_iteration(&conn, iter_id, "failed", None, Some("test error")).unwrap();
        // Reset task to pending so it can be picked up again
        conn.execute("UPDATE tasks SET status = 'pending' WHERE id = ?1", [task_id]).unwrap();
    }

    // Verify counters
    let (total_attempts, total_failures): (i64, i64) = conn.query_row(
